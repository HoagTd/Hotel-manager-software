...
public class GUIThongKePhong extends JPanel implements ActionListener{
	/**
	 * 
	 */
	...
	public GUIThongKePhong(Frame cha) {
		con = cha;
		...
		pNorth1.add(lblTitle = new JLabel("THÔNG KÊ DOANH THU PHÒNG"));
...
//		b1.setBorder(BorderFactory.createLineBorder(Color.red));
		...
		pPhong.setPreferredSize(new Dimension(800, 515));
		b.add(b1 = Box.createHorizontalBox());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o.equals(btnTheoNgay)) {
			if(cboNgay.getSelectedItem().toString().equals("") || cboThang.getSelectedItem().toString().equals("") || cboNam.getSelectedItem().toString().equals(""))
				...
				JFreeChart chartPhong = ChartFactory.createBarChart("Thống kê doanh thu phòng", "Ngày", "Số tiền", chartDataPhong,PlotOrientation.VERTICAL,true,true,false);
...
//				pTong.validate();
			...
			if(cboThang.getSelectedItem().toString().equals("") || cboNam.getSelectedItem().toString().equals(""))
				...
				for(int i = 1; i < 32; i++) {
					...
					double doanhThuPhong = qltk.layDoanhThuPhongTheoNgay(ngayPhong);
					chartDataPhong.setValue(doanhThuPhong, "Doanh thu",i+"");
				}
				JFreeChart chartPhong = ChartFactory.createBarChart("Thống kê doanh thu phòng tháng " + cboThang.getSelectedItem().toString()+" năm "+cboNam.getSelectedItem().toString(), "Ngày", "Số tiền", chartDataPhong,PlotOrientation.VERTICAL,true,true,false);
				CategoryPlot plotPhong = chartPhong.getCategoryPlot();
				plotPhong.setRangeGridlinePaint(Color.red);
				ChartPanel chartPanelPhong = new ChartPanel(chartPhong);
				chartPanelPhong.setPreferredSize(new Dimension(800, 500));
